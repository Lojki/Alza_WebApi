...
namespace Alza_WebApi.Api.V1.Controller
{
    /// <summary>
    /// Product controller.
    /// </summary>
    ...
    public class ProductController : ControllerBase
    {
        ...
        /// </summary>
        /// <param name="cancellationToken">Cancellation token.</param>
        /// <returns>An <see cref="IEnumerable{T}"/> that contains elements with all <see cref="Product"/>s.</returns>
        [HttpGet("AllProducts")]
        [MapToApiVersion("1")]
        [SwaggerOperation("Get all products", "Get all products")]
        [SwaggerResponse(StatusCodes.Status200OK)]
        public async Task<IEnumerable<Product>> GetAllProducts(CancellationToken cancellationToken)
        {
            LogCall(nameof(GetAllProducts));

            return (await _productRepository.GetAllProducts(cancellationToken)).Select(ProductMap.MapToResponseModel);
        }
