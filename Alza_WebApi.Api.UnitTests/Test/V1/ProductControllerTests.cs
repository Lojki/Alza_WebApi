...
using Alza_WebApi.Api.UnitTests.V1.TestObject;
...
namespace Alza_WebApi.Api.UnitTests.Test.V1
{
    public class ProductControllerTests : BackendTestBase
    {
        ...
        [Fact]
        public async Task ProductController_GetAllProducts_OK()
        {
            // arrange
            var products = new List<Data.Model.Product>
            {
                DataTestObject.Product,
                DataTestObject.NotAvailableProduct
            };

            var productRepositoryMock = Substitute.For<IProductRepository>();
            productRepositoryMock.GetAllProducts(Arg.Any<CancellationToken>())
                .Returns(products);
            ...
            using (var client = server.CreateClient())
            {
                ...
                Assert.All(result, r => products.Select(p => p.Name).Contains(r.Name));
            }
        }
